...
$(document).ready(function() {
  ...
  $.ajax
    ...
    $("#submitQueue").click(function() {
          ...
          var i;
          for (i = 0; i < queueNumber.length; i++ )
          {
            alert(queueNumber[i]);
            if(queueNumber == queueNumber[i])
            {
              swal("The number already exists! Click on recall or complete button.");
            }
            if ($("#queueBox > div").length > 3)
            {
              swal("Please complete previous numbers before entering a new number!");
            }
            else{
              //function for adding to DB
                 ...
                 success: function(response){
                   $('#queueBox').append('<div id="div'+ queueNumber +'"><div class="row"><div class="col-sm-7 col-md-7">' + queueNumber +  ' </div><div class="col-sm-5 col-md-5"><img src="images/recall_btn.png" height="45%" width="45%" id="recallBtn" onclick="recallNumber('+queueNumber+')"/></button>' + ' ' + '<img src="images/complete_btn.png" height="45%" width="45%" class="deleteBtn" onclick="deleteQueueNumber('+queueNumber+')"/> </div></div></div>');
                   $('#queueNumber').empty();
